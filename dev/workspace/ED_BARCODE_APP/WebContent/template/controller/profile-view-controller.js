...
module.controller('profile.view.controller', function($scope, $rootScope, $mdDialog, $location, $profileService, $localProfileService) {
	...
	function convertDateThai(myDate) {
		...
		var newDate1 = [ spDate[0].substr(6, 2), spDate[0].substr(4, 2), (parseInt(date1) + 543).toString() ];
		var newDate2 = [ spDate[1].substr(6, 2), spDate[1].substr(4, 2), (parseInt(date2) + 543).toString() ];

		// console.log(newDate1,newDate2 );

		return newDate1.join("/") + " - " + newDate2.join("/");
	}
